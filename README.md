...
**CI and Vercel never run migrate/seed.** Apply schema and demo data yourself in the Supabase SQL Editor using **[server/db/supabase/](server/db/supabase/)** (in order). See that folder’s README for the pooler URL, `CHANGE_ME` password, and Vercel vars:
